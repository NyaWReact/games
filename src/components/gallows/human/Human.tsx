...
const Human = () => {
  ...
  const {mistakes, word, letters, isShow, setIsShow, setLetters,setMistakes, setWord} = useGallowsContext()
  ...
  useEffect(() => {
    ...
    for (let i= 0; i < word.length; i++) {
      if (letters.includes(word[i])) {
        setOpenWord(openWord => openWord + 1)
      }  
    }

  }, [mistakes, letters, word, setIsShow])

  useEffect(() => {
    if (openWord === word.length) {
      setType('alive')
      setIsShow(true)
    }
  }, [openWord, word, setIsShow]
)
  ...
  console.log(mistakes, word, type, isShow);
  
  return (
    <div className={styles.human_wrapper}>
      <div className={clsx(styles.human, isShow && (type === 'lose' ? styles.died : styles.alive))}>
      {isShow ? <div className={styles.message}>
        {type === 'lose' ? `You lose. Right word: ${word.join('')}` : 'You win'}

      </div> :
      <div className={styles.block}>
        <div className={styles.person}>
          {mistakes >= 1 && <div className={styles.first}/>}
          {mistakes >= 1 && <div className={styles.second}/>}
          {mistakes >= 2 && <div className={styles.third}/>}
          {mistakes >= 3 && <div className={styles.fourth}/>}
          {mistakes >= 4 && <div className={styles.fifth}/>}
          {mistakes >= 5 && <div className={styles.sixth}/>}
          {mistakes >= 6 && <div className={styles.seventh}/>}
          {mistakes >= 7 && <div className={styles.eighth}/>}
          {mistakes >= 8 && <div className={styles.ninth}/>}
          {mistakes >= 9 && <div className={styles.tenth}/>}
          {mistakes >= 10 && <div className={styles.eleventh}/>}
      ...
      <VscDebugRestart className={styles.restart} onClick={() => restart()}/>
